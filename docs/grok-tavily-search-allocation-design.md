...
| 可接受操作 | 现有本地配置、模型和输入校验完成后，并发启动一次 Grok Search；Tavily 启用且有 key 时固定启动一次 Tavily Search standby；Tavily Extract/Map 保持显式调用 |
...
| 用户明确完成 | 通过 `extra_sources` 决定 Grok 成功路径公开和缓存多少补充信源；需要全文深读时显式调用 `web_fetch`，需要站点映射时显式调用 `web_map` |
...
| 规模与时长 | 单次工具调用；Grok 调度一次，已配置的 Tavily Search standby 最多调度一次，名义 provider 阶段上限为 `max(30, 30)=30` 秒，而非相加 |
...
现有补充来源配额必须原样保留，并与 Tavily standby 请求分离：
...
- `extra_sources=0` 时不公开或缓存补充信源，也不调用 Firecrawl。
- Tavily 启用且有 key 时，无论 Tavily 补充配额是否为零，仍请求一次 standby；`max_results` 为 `max(5, tavily_count)`。这只规定请求至少 5 个备用候选，不要求上游必须返回至少 5 条。

Tavily 未启用、无 key、失败或没有可用项时，Grok 不可用必须成为明确工具错误。双配置时 Firecrawl 仍取得全部补充配额并进入缓存，但 Tavily standby 仍可提供回退正文。不得改造补充配额，也不得让 Firecrawl 提供 fallback body。
...
2. Tavily 启用且有 key 时创建一次 Tavily Search standby，请求 `max(5, tavily_count)` 个候选，并用 `asyncio` 整操作 deadline 限制为 30 秒。
...
6. 全槽取消扫描通过后，才按严格规则分类 Grok 槽位并选择 Grok 或已完成的完整 Tavily standby 内容；Grok 成功路径只从其中选取 `tavily_count` 条作为补充信源。

```text
validate locally before creating provider coroutines
  |-- Grok Search once, whole-operation deadline 30s
  |-- configured Tavily Search standby once, max_results=max(5, quota), deadline 30s
...
Grok nonblank ----------------------> Grok content + quota-selected merged sources
Grok unavailable + usable Tavily ---> deterministic full standby fallback body
Grok unavailable + no Tavily ------> explicit tool error
any child/outer cancellation ------> propagate
local/programming error -----------> propagate
```

这不是先等待 Grok、再调用 Tavily 的串行流程。两者在校验后同时开始；Tavily 通常先完成并被保留，Grok 非空时获胜且只有配额内结果可进入 sources，Grok 不可用时选择已经完成的完整 Tavily standby 结果作为正文。

| Grok 槽位终态 | Tavily 终态 | 行为 |
| --- | --- | --- |
| 非空白 `str` | 任意非取消终态 | 返回 Grok 内容和现有合并 sources，`content_source="grok"` |
| 空白/whitespace、自身整操作 `TimeoutError`，或合法请求边界产生的 `httpx.HTTPError` | 有可用 standby 原始结果 | 使用同批完整 Tavily Search standby 结果回退，`content_source="tavily_search_fallback"`；sources 仍只含补充配额内结果与 Firecrawl |
| 同上不可用终态 | Tavily 未配置、空、失败或无可用项 | 抛出明确工具错误，绝不返回空白成功 |
...
`content_source` 只允许 `grok` 或 `tavily_search_fallback`。既有 `session_id`、`content`、`sources_count` 字段保留；source 合并、缓存和 `get_sources` 继续只反映 `extra_sources` 选中的 Tavily 补充结果与既有 Firecrawl 结果，standby-only 结果不进入这些字段。必须保持 complete existing baseline tool set 的名称与输入 schema；`extra_sources` 描述明确 `0` 只关闭补充信源，不关闭已配置 Tavily standby。
...
| 场景 | 必须证明的结果 |
| --- | --- |
| 并发调度 | 本地校验完成后 Grok 与已配置 Tavily standby 同时开始；每个 provider 最多调用一次 |
| Grok 返回非空白 | 返回 Grok 内容、配额内合并 sources、`content_source="grok"`；`extra_sources=0` 时 standby 不公开、不缓存、不计数 |
...
| Grok 不可用且 Tavily 缺席/禁用/空/错误 | 每种代表性路径均为明确工具错误 |
| standby 与配额 | 缺省或显式 `extra_sources=0` 仍调一次 Tavily，`max_results=5`；更大 Tavily 配额增加请求数；双配置补充配额仍全给 Firecrawl |
| 格式化器 | 直接使用 raw Tavily 的 title/url/content；过滤、trim、URL 首次去重、顺序、1500/8000 上限、非空与标签确定 |
| source/cache | standby 与公开配额分离；Firecrawl-first 合并、缓存及 `get_sources` 保持不变；Firecrawl 不进入 fallback body |
...
| 实例边界 | 先前 quick smoke 只覆盖前一实现；当前 standby 改动尚未获目标实例验证，不证明生产可靠性 |
...
Grok Search 与已配置的 Tavily Search standby 并发，各自固定 30 秒整操作 deadline，因此 provider 阶段名义上限为 `max(30, 30) = 30` 秒。`web_search` 之前仍有可选、未缓存的显式模型 `/models` 校验，当前 HTTPX 上限为 10 秒；可选校验最多 10 秒加并发 provider 阶段最多 30 秒，目标路径名义合计约 40 秒，相对 `USER_OBSERVATION` 的外层 60 秒约有 20 秒余量。该数值是产品预算和名义边界，不是端到端 SLA。模型缓存锁没有显式 deadline，是保留风险，本次不增加修复。

上述不是整个 MCP 的硬性 60 秒 SLA：Firecrawl、锁等待、调度以及其他既有前后处理可增加总时长。目标保证只覆盖已配置 Tavily standby 的目标部署与本次规定的 provider 操作。

2026-09-12 的已授权目标 quick smoke 只覆盖前一实现中 `extra_sources=1` 的正常与受控回退路径。当前 standby 改动尚未执行实例测试；后续实例测试仍须单独授权并复用下述 runbook，且不得把旧结果扩展为当前实现、load/stress、生产可靠性或端到端 SLA 证明。
...
- Firecrawl 时序不在 60 秒保证内；双配置仍可能等待 Firecrawl，`web_fetch` 的顺序 Firecrawl fallback 也可能超过外层预算。
- 非空但不完整的 Grok 流不会触发回退。
- Tavily 禁用、无 key 或无可用项时不存在 standby fallback；`extra_sources=0` 不关闭已配置 Tavily standby。
- Firecrawl 调用与配额行为保持不变，且永不提供 fallback body。

## INSTANCE_TEST_RUNBOOK

- 适用基线：版本 `0.2.1`、`server.py` SHA256 `58d23341c4a09a5617782d4d94d6e5a43182ad7fc0a2ea06b1b5ba6a7e30a1c4`、Tavily-only/no Firecrawl；凭据与服务地址仅由现有 Grok/Tavily 环境变量在进程中提供，文档和产物不得记录值；默认模型 `grok-chat-fast` 必须可用。
...
- 稳定接口：使用真实 FastMCP Client 调用 `web_search`，省略 `extra_sources` 且不显式传 model/platform。正常场景必须非 error，session/content 非空，`content_source="grok"` 且 `sources_count=0`；目标记录须另证 Tavily Search 恰好调用一次且 `max_results=5`。
- 受控回退：仅在测试进程内把 `GROK_API_URL` 指向 loopback refused endpoint，保留真实 Tavily 与 no Firecrawl 配置；省略 `extra_sources` 调用同一接口后必须非 error、content 非空、`content_source="tavily_search_fallback"`、`sources_count=0`，并同时含两条规定的 fallback 标签。
...
- `FACT`：前一实现位于版本 `0.2.1`、运行时补丁 SHA256 `8673047f0d4ac83c13e94c920b3bb39ff8f1928e8d3c4aae3e6ed766cfa19340`；当时 `server.py` SHA256 为 `64da9bd573702cd14a6bad48b078b9ea92d91a70955c432406bd690e28c33054`。当前 standby 改动会使该指纹失效，因此这条事实只作为修改前基线，不证明新实现。
- `FACT`：前一实现曾在 Python 3.12.4 环境通过 30/30 聚焦与完整发现 suite、源码/测试 `compileall`、FastMCP 七工具 metadata/schema A/B 和静态交付门。其测试文件 SHA256 为 `aa55d0dfe095ffefc01487d5fd4a84ec768be598400416f67956ee3829c2f92d`；当前源码和测试变化后这些结论失效，须按验收命令重新验证。
- `FACT`：2026-09-12 的目标 quick smoke 运行在前一补丁 `8673047f0d4ac83c13e94c920b3bb39ff8f1928e8d3c4aae3e6ed766cfa19340` 上，证明当时 `extra_sources=1` 的正常与受控回退路径。当前默认/零配额 standby 行为没有实例证据；旧 smoke 不证明它，也不证明 load/concurrency、生产可靠性/SLA、所有上游失败或 Grok2API 根因。
...
| standby 修复在 `extra_sources` 缺省/零时仍调用一次 Tavily，且与公开/cache 配额分离 | `EVIDENCE_GAP` | 已完成源码与回归测试修改，但本实施批次按约束未运行测试、构建或实例验证；须由独立测试阶段核验 |
