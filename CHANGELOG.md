...
Grok SSE fix source/test diff artifact: `399a8c44d30bee4c9a9ff90a353e4d150ab0ebda6fcedf90b8e6d41ce28c29d8` (SHA-256).
...
- Fixed: HTTP 200 SSE top-level Chat Completions errors and Responses `response.failed` events with `response.error` now propagate immediately as explicit provider errors containing only their code, type, and message instead of being treated as empty successes; transport and HTTP status retries are unchanged.
...
- Verified the Grok SSE fix: the top-level and Responses same-shape pre-fix probes were `RED_CONFIRMED` and `RED_CONFIRMED_RESPONSE_FAILED`; after the fix, the complete existing `tests/test_regressions.py` suite collected six tests and passed 6/6 with exit 0, `py_compile` exited 0, and `git diff --check` exited 0.
